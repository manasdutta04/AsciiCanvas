...
export function cloneCells(cells: Cells): Cells { return new Map(cells); }
...
export function selectionBounds(s: Selection) {
  return {
    minC: Math.min(s.startC, s.endC), maxC: Math.max(s.startC, s.endC),
    minR: Math.min(s.startR, s.endR), maxR: Math.max(s.startR, s.endR),
  };
}
...
export const FONT_FAMILY = "'Cascadia Code','Fira Code','Consolas','Courier New',monospace";
