...
export function StatusBar({
  tool, col, row, zoom, onZoomChange, onZoomFit, onHome,
  cellCount, canUndo, canRedo, onUndo, onRedo,
}: Props) {
    ...
    <div style={styles.bar}>
      ...
      <div style={styles.right}>
        ...
        <div style={styles.zoomRow}>
          <ZoomBtn label="−" onClick={() => onZoomChange(Math.max(0.15, zoom * 0.85))} />
          <button style={styles.zoomPct} onClick={onHome} title="Reset view (Home)">
            {pct}%
          </button>
          <ZoomBtn label="+" onClick={() => onZoomChange(Math.min(5, zoom * 1.18))} />
        </div>
        <button onClick={onZoomFit} style={styles.fitBtn} title="Zoom to fit (Ctrl+Shift+F)">fit</button>
      </div>
    </div>
  );
}
...
function ZoomBtn({ label, onClick }: { label: string; onClick: () => void }) {
  return <button onClick={onClick} style={styles.zoomBtn}>{label}</button>;
}

const styles: Record<string, React.CSSProperties> = {
  bar: {
    height: 28, background: "#0d0d18", borderTop: "1px solid #1e1e2e",
  ...
  right: { display: "flex", alignItems: "center", gap: 4, minWidth: 180, justifyContent: "flex-end" },
  toolLabel: { color: "#89b4fa", fontWeight: 700, letterSpacing: 0.5, textTransform: "uppercase", fontSize: 10 },
  sep:       { color: "#2a2a3a" },
  dim:       { color: "#45475a" },
  ...
  zoomRow: {
    ...
    background: "#1a1a28", borderRadius: 5, padding: "1px 3px",
    border: "1px solid #252535",
  },
  zoomBtn: {
    background: "none", border: "none", color: "#45475a",
    fontSize: 13, cursor: "pointer", padding: "0 3px", lineHeight: 1,
  },
  zoomPct: {
    background: "none", border: "none", color: "#7f849c",
  ...
  fitBtn: {
    ...
    color: "#45475a", fontSize: 10, cursor: "pointer",
